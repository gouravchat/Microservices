...
class OrderClient:
    ...
    def get_order():
        ...
        response = requests.request(method="GET", url='http://192.168.0.104:8083/api/order', headers=headers)
        order = response.json()
        return order
    ...
    def update_order(items):
        ...
        url = 'http://192.168.0.104:8083/api/order/update'
        headers = {
            'Authorization': 'Basic ' + session['user_api_key']
        }
    ...
    def post_add_to_cart(product_id, qty=1):
        ...
        url = 'http://192.168.0.104:8083/api/order/add-item'
        headers = {
            'Authorization': 'Basic ' + session['user_api_key']
        }
    ...
    @staticmethod
    def post_checkout():
        url = 'http://192.168.0.104:8083/api/order/checkout'
        headers = {
            'Authorization': 'Basic ' + session['user_api_key']
        }
